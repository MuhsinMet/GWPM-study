...
import xarray as xr
import numpy as np
from config import models, reference_data, variables, start_date_str, end_date_str, param  # Import from config
from datetime import datetime, timedelta
from scipy.stats import pearsonr

# User inputs (now imported from config)
# start_date_str, end_date_str, and param should be imported from config
start_date = datetime.strptime(start_date_str, "%Y%m%d")
end_date = datetime.strptime(end_date_str, "%Y%m%d")

forecast_horizons = list(range(1, 15))  # Example: 1-15 days forecast horizon

# Initialize dictionaries for RMSE and correlation calculations
...
while current_date <= end_date:
    ...
    for horizon in forecast_horizons:
        ...
        # Load reference data (ERA5 or MSWEP)
