...
# User inputs
start_date_str = '20240922'  # Start date
end_date_str = '20240925'    # End date
param = 'Temp'  # Parameter to analyze ('Temp', 'P', 'RelHum', 'Wind')
reference_choice = 'GDAS'  # Choose between 'ERA5' and 'GDAS'
...
# Initialize dictionaries to store aggregated RMSE and correlation values
rmse_aggregated = {horizon: {model_name: 0 for model_name in models if param in models[model_name]['predictors']} for horizon in forecast_horizons}
correlation_aggregated = {horizon: {model_name: 0 for model_name in models if param in models[model_name]['predictors']} for horizon in forecast_horizons}
...
while current_date <= end_date:
    ...
    for horizon in forecast_horizons:
        ...
        # Access paths to forecast data for models
        model_paths = {
...
# Plot RMSE
for idx, model_name in enumerate([m for m in models if param in models[m]['predictors']]):
...
plt.savefig(f'RMSE_and_Correlation_Comparison_{param}_{reference_choice}_{start_date_str}_to_{end_date_str}.png')
plt.show()
